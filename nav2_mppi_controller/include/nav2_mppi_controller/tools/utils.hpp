 ...
 * @param marker_id_start Starting marker ID
 * @param trajectory_step Step size for selecting trajectories (spatial downsampling)
 * @param time_step Step size for selecting points along trajectories (temporal downsampling)
 * @return MarkerArray containing trajectory line strips
 */
inline visualization_msgs::msg::MarkerArray createTrajectoryMarkers(
 ...
 * @param time_step Step size for downsampling footprints along trajectory
  ...
  size_t time_step)
{
  ...
  int marker_id = 0;
  for (int i = 0; i < trajectory.rows(); i += time_step) {
